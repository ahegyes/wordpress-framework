...
### CI: PHP checks via the reusable composer-script-matrix workflow (2026-06-29)
...
This shape is load-bearing for PHPStan: it runs seven per-package configs that each isolate one package's dependency/stub surface (only `woocommerce` and `utilities` scan the WooCommerce stubs — see the static-analysis-stubs decision), and a single `phpstan analyse -c <one-config>` run cannot reproduce that isolation, so `composer lint:php:phpstan` (the seven-run sweep) is the unit the workflow invokes. `reusable-php-lint` generalized and replaced the former fixed-phpcs-plus-phpstan `reusable-php-qa` (removed across the workspace); delegating to consumer composer scripts is the de-facto standard among the configs reusable workflows, with the policy-enforcing ones (supply-chain audit, schema conformance, release) deliberately keeping their command hardcoded so a consumer cannot weaken them.
