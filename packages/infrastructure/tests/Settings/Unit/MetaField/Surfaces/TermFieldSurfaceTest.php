...
final class TermFieldSurfaceTest extends TestCase {
	...
	private TermFieldSurface $store;

	protected function setUp(): void {
		parent::setUp();

		$this->repository = new InMemoryObjectMetaRepository();
		$this->store      = new TermFieldSurface( repository: $this->repository );
	}
	...
	public function test_a_value_round_trips_under_the_resolved_storage_key(): void {
		...
		$this->store->set( $group, 5, 'note', 'hello' );

		self::assertSame( 'hello', $this->store->get( $group, 5, 'note' ) );
		self::assertSame( 'hello', $this->repository->get( 5, 'note' ) );
		self::assertFalse( $this->store->has( $group, 6, 'note' ) );
	}
	...
	public function test_a_meta_key_override_is_the_byte_exact_storage_key(): void {
		...
		self::assertSame( 'hello', $this->repository->get( 5, '_dws_note' ) );
		self::assertFalse( $this->repository->has( 5, 'note' ) );
	}
	...
	public function test_get_returns_the_caller_fallback_never_the_field_default_when_nothing_is_stored(): void {
		...
		self::assertSame( 'fallback', $this->store->get( $group, 5, 'note', 'fallback' ) );
	}
	...
	public function test_set_stores_a_checkbox_in_its_canonical_yes_no_form(): void {
		...
		$this->store->set( $group, 5, 'flag', true );
		self::assertSame( 'yes', $this->repository->get( 5, 'flag' ) );

		$this->store->set( $group, 5, 'flag', false );
		self::assertSame( 'no', $this->repository->get( 5, 'flag' ) );
		self::assertTrue( $this->store->has( $group, 5, 'flag' ) );
	}
	...
	public function test_set_revokes_the_key_for_a_value_a_form_save_would_not_store(): void {
		...
		$this->store->set( $group, 5, 'note', 'hello' );
		$this->store->set( $group, 5, 'note', '' );

		self::assertFalse( $this->store->has( $group, 5, 'note' ) );
	}
	...
	public function test_delete_removes_a_stored_value_and_reports_a_missing_one(): void {
		...
		self::assertTrue( $this->store->delete( $group, 5, 'note' ) );
		self::assertFalse( $this->store->has( $group, 5, 'note' ) );
		self::assertFalse( $this->store->delete( $group, 5, 'note' ) );
	}
	...
	public function test_a_field_the_group_does_not_declare_is_rejected(): void {
		...
		$this->store->has( $this->group( new SettingsField( id: 'note', type: 'text', label: 'Note' ) ), 5, 'missing' );
	}
