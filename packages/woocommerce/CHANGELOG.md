...
Pending entries live in [`changelog/`](./changelog) — add via `composer packages:woocommerce:changelog:add` from the monorepo root. Aggregate into a release with `composer packages:woocommerce:changelog:write`.
