...
final class OrderFieldSurfaceTest extends TestCase {
	...
	private OrderFieldSurface $surface;

	protected function setUp(): void {
		parent::setUp();

		$this->repository = new InMemoryObjectMetaRepository();
		$this->surface    = new OrderFieldSurface( repository: $this->repository );
	}
	...
	public function test_a_value_round_trips_under_the_resolved_storage_key(): void {
		...
		$this->surface->set( $group, 11, 'note', 'hello' );

		self::assertSame( 'hello', $this->surface->get( $group, 11, 'note' ) );
		self::assertSame( 'hello', $this->repository->get( 11, 'note' ) );
		self::assertFalse( $this->surface->has( $group, 12, 'note' ) );
	}
	...
	public function test_get_returns_the_caller_fallback_never_the_field_default_when_nothing_is_stored(): void {
		...
		self::assertSame( 'fallback', $this->surface->get( $group, 11, 'note', 'fallback' ) );
	}
	...
	public function test_set_stores_a_checkbox_in_its_canonical_yes_no_form(): void {
		...
		$this->surface->set( $group, 11, 'flag', true );
		self::assertSame( 'yes', $this->repository->get( 11, 'flag' ) );

		$this->surface->set( $group, 11, 'flag', false );
		self::assertSame( 'no', $this->repository->get( 11, 'flag' ) );
		self::assertTrue( $this->surface->has( $group, 11, 'flag' ) );
	}
	...
	public function test_set_revokes_the_key_for_a_value_a_form_save_would_not_store(): void {
		...
		$this->surface->set( $group, 11, 'note', 'hello' );
		$this->surface->set( $group, 11, 'note', '' );

		self::assertFalse( $this->surface->has( $group, 11, 'note' ) );
	}
	...
	public function test_delete_removes_a_stored_value_and_reports_a_missing_one(): void {
		...
		self::assertTrue( $this->surface->delete( $group, 11, 'note' ) );
		self::assertFalse( $this->surface->has( $group, 11, 'note' ) );
		self::assertFalse( $this->surface->delete( $group, 11, 'note' ) );
	}
	...
	public function test_a_field_the_group_does_not_declare_is_rejected(): void {
		...
		$this->surface->get( $this->group( new SettingsField( id: 'note', type: 'text', label: 'Note' ) ), 11, 'missing' );
	}
	...
	public function test_meta_keys_enumerates_the_resolved_storage_keys(): void {
		...
		self::assertSame( array( 'note', '_dws_ref' ), $this->surface->meta_keys( $group ) );
	}
