...
final class OrderFieldSurfaceTest extends TestCase {
	...
	public function test_save_preserves_an_existing_value_when_a_present_submission_is_invalid(): void {
		...
		$_POST = array(
			$this->nonce_name() => $this->nonce(),
			self::GROUP_ID      => array( 'status' => 'open' ),
		);
		\do_action( 'woocommerce_process_shop_order_meta', $this->order_id );
	...
	public function test_save_is_skipped_for_a_user_without_the_order_capability(): void {
		$subscriber = \wp_insert_user(
			array(
				'user_login' => 'dws_sub_' . $this->order_id,
				'user_pass'  => 'x',
				'role'       => 'subscriber',
			),
		);
	...
	public function test_the_box_is_not_added_for_a_user_who_cannot_edit_the_order(): void {
		...
		$subscriber = \wp_insert_user(
			array(
				'user_login' => 'dws_sub_render_' . $this->order_id,
				'user_pass'  => 'x',
				'role'       => 'subscriber',
			),
		);
	...
	public function test_a_custom_field_type_renders_and_saves_through_the_order_surface(): void {
		...
		$_POST = array(
			$this->nonce_name() => $this->nonce(),
			self::GROUP_ID      => array( 'home_page' => '42' ),
		);
		\do_action( 'woocommerce_process_shop_order_meta', $this->order_id );
	...
	public function test_clearing_a_field_with_a_default_revokes_it_without_restoring_the_default(): void {
		...
		$_POST = array(
			$this->nonce_name() => $this->nonce(),
			self::GROUP_ID      => array( 'note' => 'typed' ),
		);
		\do_action( 'woocommerce_process_shop_order_meta', $this->order_id );
